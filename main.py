...
# Convert date input (returns datetime.date) into datetime.datetime
start_date = datetime.combine(st.date_input("Start Date", datetime(2025, 5, 9).date()), time.min)
end_date = datetime.combine(st.date_input("End Date", datetime(2017, 1, 1).date()), time.min)
